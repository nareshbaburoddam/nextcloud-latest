...
use OCA\Notifications\Vendor\GuzzleHttp\Client;
...
class WebPush
{
    ...
    public function flush(?int $batchSize = null): \Generator
    {
        ...
        foreach ($batches as $batch) {
            ...
            foreach ($requests as $request) {
                $promises[] = $this->client->sendAsync($request)->then(function ($response) use ($request) {
    ...
    public function flushPooled(callable $callback, ?int $batchSize = null, ?int $requestConcurrency = null): void
    {
        ...
        foreach ($batches as $batch) {
            ...
            $pool = new Pool($this->client, $batch, ['concurrency' => $requestConcurrency, 'fulfilled' => function (ResponseInterface $response, int $index) use ($callback, $batch): void {
    ...
    protected function prepare(array $notifications): array
    {
        ...
        foreach ($notifications as $notification) {
            ...
            if (!empty($payload) && !empty($userPublicKey) && !empty($userAuthToken)) {
                ...
                $headers = ['Content-Type' => $options['contentType'], 'Content-Encoding' => $contentEncoding];
