    ...
    public function analyze(JWK $jwk, MessageBag $bag): void
    {
        ...
        if ($jwk->has('key_ops')) {
            ...
            if (!is_array($key_ops)) {
                ...
                if ($unsupportedOps !== []) {
                    $bag->add(Message::high(sprintf('The parameter "key_ops" contains unsupported values: "%s". Please use only the following values: %s.', implode('", "', $unsupportedOps), implode(', ', $allowedOps))));
                }
            }
        }
    }
