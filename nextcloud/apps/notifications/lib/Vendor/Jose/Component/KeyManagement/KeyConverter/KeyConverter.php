...
namespace OCA\Notifications\Vendor\Jose\Component\KeyManagement\KeyConverter;

use OCA\Notifications\Vendor\Brick\Math\BigInteger;
use InvalidArgumentException;
use OCA\Notifications\Vendor\Jose\Component\Core\Util\Base64UrlSafe;
...
use OCA\Notifications\Vendor\SpomkyLabs\Pki\CryptoTypes\Asymmetric\PrivateKey;
use OCA\Notifications\Vendor\SpomkyLabs\Pki\CryptoTypes\Asymmetric\PublicKey;
...
final readonly class KeyConverter
{
    ...
    private static function loadKeyFromPEM(string $pem, ?string $password = null): array
    {
        ...
        return match ($details['type']) {
            ...
            4 => self::tryToLoadX25519Key($details),
            // OPENSSL_KEYTYPE_X25519
            5 => self::tryToLoadED25519Key($details),
            // OPENSSL_KEYTYPE_ED25519
            6 => self::tryToLoadX448Key($details),
            // OPENSSL_KEYTYPE_X448
            7 => self::tryToLoadED448Key($details),
        ...
        try {
            ...
            switch ($key->algorithmIdentifier()->oid()) {
                ...
                case AlgorithmIdentifier::OID_X448:
                    ...
                    return ['kty' => 'OKP', 'crv' => $curve, 'x' => Base64UrlSafe::encodeUnpadded($publicKeyBits->asBitString()->string()), 'd' => Base64UrlSafe::encodeUnpadded($key->privateKeyData())];
                default:
