...
use OCA\Notifications\Vendor\Jose\Component\Core\AlgorithmManager;
use OCA\Notifications\Vendor\Jose\Component\Core\JWK;
use OCA\Notifications\Vendor\Jose\Component\Core\JWKSet;
use OCA\Notifications\Vendor\Jose\Component\Core\Util\KeyChecker;
use OCA\Notifications\Vendor\Jose\Component\Encryption\Algorithm\ContentEncryptionAlgorithm;
use OCA\Notifications\Vendor\Jose\Component\Encryption\Algorithm\KeyEncryption\DirectEncryption;
use OCA\Notifications\Vendor\Jose\Component\Encryption\Algorithm\KeyEncryption\KeyAgreement;
use OCA\Notifications\Vendor\Jose\Component\Encryption\Algorithm\KeyEncryption\KeyAgreementWithKeyWrapping;
use OCA\Notifications\Vendor\Jose\Component\Encryption\Algorithm\KeyEncryption\KeyEncryption;
use OCA\Notifications\Vendor\Jose\Component\Encryption\Algorithm\KeyEncryption\KeyWrapping;
...
class JWEDecrypter
{
    ...
    private function decryptPayload(JWE $jwe, string $cek, ContentEncryptionAlgorithm $content_encryption_algorithm): string
    {
        return $content_encryption_algorithm->decryptContent($jwe->getCiphertext() ?? '', $cek, $jwe->getIV() ?? '', $jwe->getAAD(), $jwe->getEncodedSharedProtectedHeader(), $jwe->getTag() ?? '');
    }
